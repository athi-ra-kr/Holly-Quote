...
# Helper: build subunits list for json_script (safe JS serialization)
def _subunits_json():
    return list(SubUnit.objects.values('id', 'name', 'unit_id'))

# Passes subunits_json for safe json_script usage in template
def material_list(request):
    return render(request, 'material_list.html', {
        'materials': Material.objects.all(),
        'units': Unit.objects.all(),
        'subunits_json': _subunits_json(),
    })

# Saves sub_unit on create
def material_add(request):
    if request.method == "POST":
        Material.objects.create(
            unit_id=request.POST.get('unit'),
            sub_unit_id=request.POST.get('sub_unit') or None,
            name=request.POST.get('name'),
...
def quotation_detail(request, id):
    ...
    if request.method == "POST":
        ...
        material_id = request.POST.get('material')   # may be empty for nos
        ...
        # ── Determine rate based on calculation type ──────────────────
        if calc_type == 'nos':
            # For nos: use the item's own stored rate, no material needed
            item_obj     = get_object_or_404(Item, id=item_id)
            current_rate = float(item_obj.rate or 0)
            mat_obj      = None   # no material for nos
        else:
        ...
        if calc_type == 'sqft':
            ...
            base_quantity = 1   # rate × count drives the total
        else:
            base_quantity = qv
        ...
        if edit_id:
            ...
            qi.item_id      = item_id
            qi.material     = mat_obj   # will be None for nos
            qi.l_val        = l
            qi.d_val        = d
            qi.h_val        = h
            qi.qty_val      = qv
...
def load_subunits(request):
    ...
    opts = '<option value="">-- Select Sub Unit --</option>' + ''.join(
        [f'<option value="{s.id}">{s.name}</option>' for s in subs]
    )
...
def load_items(request):
    ...
    opts  = '<option value="">-- Select Item --</option>' + ''.join(
        [f'<option value="{i.id}" data-type="{i.calculation_type}" data-item-rate="{i.rate or 0}">{i.name}</option>'
         for i in items]
    )
    return HttpResponse(opts)
